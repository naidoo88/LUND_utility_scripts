"""
Parse events from LUND files and then write them to a single ROOT file using uproot.

Usage: 
python3 LUND_to_ROOT.py -i /source/files/path -o /output/file/path  -p "*.dat"
...
def LUND_to_ROOT(
    ...
    with uproot.recreate(output_file_path) as root_file:
        for file in source_files:
            ...
            #Create tree in the first instance
            if N_files == 1:
                root_file["LUND"] = lund_df
            #Append to thereafter
            else:
                root_file["LUND"].extend(lund_df)
